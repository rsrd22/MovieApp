...
class MovieRepositoryImpl(
    private val dataSourceRemove: RemoveMovieDataSource,
    private val dataSourceLocal: LocalMovieDataSource
) : MovieRepository {

    override suspend fun getUpcomingMovies(): MovieList {
        return if(InternetCheck.isNetworkAvailable()) {
            dataSourceRemove.getUpcomingMovies().results.forEach { movie ->
    ...
    override suspend fun getTopRatedMovies(): MovieList {
        return if(InternetCheck.isNetworkAvailable()) {
            dataSourceRemove.getTopRatedMovies().results.forEach { movie ->
    ...
    override suspend fun getPopularMovies(): MovieList {
        return if(InternetCheck.isNetworkAvailable()) {
            dataSourceRemove.getPopularMovies().results.forEach { movie ->
